...
|Joinpoint|**`Advice`를 적용할 수 있는 후보 지점 혹은 호출 이벤트**<br>관심사를 구현한 코드를 끼워 넣을 수 있는 프로그램의 이벤트를 말하며, 예로는 call events, execution events, initialization events 등이 있음|  
|Pointcut|언제 `Advice`를 실행할지를 정의<br>`Target` 클래스와 `Advice`가 결합(`Weaving`)될 때 둘 사이의 결합규칙을 정의<br>**특정 조건에 의해 필터링된 조인포인트**, 수많은 조인포인트 중에 특정 메소드에서만 횡단 공통기능을 수행시키기 위해서 사용한다.| 
